...
(defn log-code-exceptions [code exception exception-details ex-triage triage-details]
  ...
  (let [log-file (str "./log/code-ex-triage/" current-time "-log-file.txt")
    ...
    (if (.endsWith log-content "]")
      (spit log-file (str (subs log-content 0 (dec (count log-content))) ", " (assoc {} :code code :exception exception :exception-details exception-details :ex-triage ex-triage :triage-details triage-details) "]") :append false) ;removes ] at end
      (spit log-file (str (assoc {} :code code  :exception-details exception-details :ex-triage ex-triage :triage-details triage-details) "]") :append true))))
...
(defn get-all-info
  ...
  (let [_ (trap-response code)]
    (when (:log? @counter)
      (let [{:keys [message modified trace exception exception-details ex-triage triage-details]} (get-tracked-errors) ;exc-via removed
            ...
            _ (log-code-exceptions code exception exception-details ex-triage triage-details)]
            all-info))))
